...
data_remove_col <- data2 %>% dplyr::select(-api_water,-api_total_impurities, -api_l_impurity, -api_ps05)
...
for (i in 1:1) {  # Looping for 5 imputations
  ...
  for (col in colnames(data_remove_col)) {
    if (col != "Target") {
      
      # Perform univariate binomial regression for each predictor
      model <- glm(Target ~ data_remove_col[[col]], data = data_remove_col, family = binomial)
...
data_remove_col <- data_remove_col %>% dplyr::select(Target, all_of(columns_to_keep))
...
full_model <- glm(Target ~ ., family = binomial, data = data_remove_col)

# Perform stepwise selection based on BIC
stepwise_model <- step(full_model, direction = "both", k = log(nrow(data_remove_col)))
...
for (interaction in interaction_terms) {
  ...
  model_with_interaction <- glm(interaction_formula, family = binomial, data = data_remove_col)
